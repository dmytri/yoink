 ...
 * @planks("the caller runs Yoink via flags")
 * @planks("the caller runs Yoink with the plan via flags")
 ...
 * @planks("Yoink prints a diagnostic for the misplaced global flag to standard error")
 * @planks("Yoink prints a diagnostic for the missing --run to standard error")
 * @planks("Yoink prints a diagnostic for the empty command to standard error")
 * @planks("Yoink prints a diagnostic for the mixed input modes to standard error")
...
async function main() {
		...
		if (arg === "--run") {
			...
			if (globalOptionsClosed) {
				process.stderr.write(
					"--pipefail must be specified before the first --run\n",
				);
				process.exitCode = 1;
				return;
			}
			...
			if (globalOptionsClosed) {
				process.stderr.write(
					"--no-pipefail must be specified before the first --run\n",
				);
				process.exitCode = 1;
				return;
			}
			...
			if (globalOptionsClosed) {
				process.stderr.write(
					"--max-bytes must be specified before the first --run\n",
				);
				process.exitCode = 1;
				return;
			}
			if (maxBytesSet) {
				process.stderr.write(
					"--max-bytes: invalid value (specified more than once)\n",
				);
				process.exitCode = 1;
				return;
			}
	 ...
	 */
	const metadata = (result: Result, index: number) =>
		Buffer.from(
			JSON.stringify({
				index,
	...
	for (let i = 0; i < results.length; i++) {
		...
		parts.push(
			Buffer.from(
				`--${boundary}${crlf}Content-Type: application/json${crlf}Content-Disposition: form-data; name="metadata"${crlf}${crlf}`,
			),
			metadata(result, i),
		);
		parts.push(
			Buffer.from(
				`${crlf}--${boundary}${crlf}Content-Type: application/octet-stream${crlf}Content-Disposition: form-data; name="stdout"${crlf}${crlf}`,
			),
			result.stdout,
		);
		parts.push(
			Buffer.from(
				`${crlf}--${boundary}${crlf}Content-Type: application/octet-stream${crlf}Content-Disposition: form-data; name="stderr"${crlf}${crlf}`,
			),
			result.stderr,
		);
	}
