...
`@logic` suite is green: 91/91 scenarios, 552/552 steps, ~14.6s. No `watchbill.json`, no live `PERTURBATION` statements, no `@captain` or `@shipwright` tags. `RIGGING.md` `eval` tier is unfitted; `broad-eval` is not run by default. Outbound `npm publish` is gated on harbour full regression per the Verification policy.
...
- **`src/cli.ts:486`** — `await realpath(command.cwd)` runs after the child has closed. If `command.cwd` was removed between plan validation and the result callback, `realpath` rejects and the bundle is lost. Crew fix: wrap in `try/catch`, fall back to the declared `command.cwd` literal so the bundle still ships. Provocation requires racing an `rm -rf` against a long-lived command; not exercised by a binding scenario today.
