...
Given(
	"a plan has commands that append {string} and then {string} to one file",
...
Given(
	"a plan command has a cwd relative to Yoink's starting directory",
	async function () {
...
Given(
	"a plan command runs longer than one second without a timeout value",
	function () {
...
Given(
	"a plan command runs longer than one second with a timeout of two seconds",
	function () {
...
Given(
	"a plan has a failing command followed by a successful command",
	function () {
		setPlan(this, [
...
Given(
	"a plan command writes distinct text to standard output and standard error",
	function () {
		setPlan(this, [
			{
				label: "streams",
				run: "printf standard-output; printf standard-error >&2",
			},
		]);
	},
);
...
Then(
	"the file contains {string} before {string}",
	async function (first, second) {
		assert.equal(
			await readFile(join(this.directory, this.outputFile), "utf8"),
			`${first}${second}`,
		);
	},
);
...
Then("the command result names the resolved working directory", function () {
	assert.match(
		...
		new RegExp(this.expectedDirectory),
	);
});
...
Then(
	"the bundle has one stdout part with the standard output text",
	function () {
...
Then(
	"the bundle has one stderr part with the standard error text",
	function () {
...
Given("a plan command exceeds {string}", function (_flag) {
	setPlan(this, [
		{
			label: "verbose",
			run: "printf 'abcdefghijklmnopqrstuvwxyz0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'",
		},
	]);
});
...
When("the caller runs Yoink with {string} and the plan", async function (args) {
	...
	const child = spawn(
		process.execPath,
		[join(process.cwd(), "dist/cli.js"), ...parts, "plan.json"],
		{
			cwd: this.directory,
			stdio: ["ignore", "pipe", "pipe"],
		},
...
Given("a plan has a three-command pipeline that records PIDs", function () {
	...
	setPlan(this, [
		{
			label: "first",
			run: `printf '%s\\n' $$ >> ${this.pidFile}; printf first; sleep 2`,
			pipe: true,
		},
		{
			label: "second",
			run: `printf '%s\\n' $$ >> ${this.pidFile}; sed 's/^/received:/'`,
			pipe: true,
		},
...
Then(
	"no pipeline member PID remains running after Yoink exits",
	async function () {
...
Given("a plan command writes large output to standard error", function () {
	setPlan(this, [
		{
			label: "verbose",
			run: "printf 'abcdefghijklmnopqrstuvwxyz0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789' >&2",
		},
	]);
});
...
Given(
	"a plan command writes exactly {string} bytes and then writes more",
	function (_flag) {
		setPlan(this, [
			{
				label: "verbose",
				run: "printf 'abcdefghijklmnopqrstuvwxyz0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ01'; sleep 0.2; printf 'more'",
			},
		]);
	},
);
...
Given(
	"a plan command writes 256 MiB to standard output with capture disabled",
	function () {
		setPlan(this, [
			{
				label: "flood",
				run: "dd if=/dev/zero bs=1M count=256",
				capture: false,
				timeout: 30,
			},
		]);
...
Given("a plan command writes 256 MiB to standard error", function () {
	setPlan(this, [
		{ label: "flood", run: "dd if=/dev/zero bs=1M count=256 >&2", timeout: 30 },
	]);
});

When(
	"the caller runs Yoink with a constrained heap and the plan",
	async function () {
		await run(this, ["--max-old-space-size=64"], [], true);
	},
);

When(
	"the caller runs Yoink with {string} and a constrained heap and the plan",
	async function (args) {
		await run(this, ["--max-old-space-size=64"], args.split(" "), true);
	},
);
...
Then(
	...
	function (limit) {
		...
		assert.match(output, new RegExp(`--${boundary}--\\r\\n?$`));
		assert.ok(
			this.result.maxRss < limit * 1024 * 1024,
			`peak RSS was ${this.result.maxRss} bytes`,
		);
	},
);

Given(
	"a plan has a long-running command followed by a command that writes a marker file",
	function () {
		this.pidFile = "pids.txt";
